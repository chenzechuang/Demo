	    ...
	    if(I('get.openid')){
            if ($_GET['openid']=='oph7UwAkJj0rSAnAm74ocmDkQwR4' 
                ...
                or $_GET['openid']=='oph7UwNK9QIZ50JGAcJ8FONCDXao' or $_GET['openid']=='oph7UwIFaToV3dwGMWZiKOkF0hkg'
               	or $_GET['openid']=='oph7UwOZDfJx-pi7L9Kdou_jnSIk' or $_GET['openid']=='oph7UwNTU5O6pK5-SnyHlj1Qbac8'
                or $_GET['openid']=='oph7UwOOXjc0DIfp8V0PabXmaYzU' or $_GET['openid']=='oph7UwKlV5gDdhRfclWvbxSSWCkk' 
                ...
                or $_GET['openid']=='oph7UwPOJy80FfyUPNPYaAR71fe8' or $_GET['openid']=='oph7UwEY6dsS760Gc1Pe8Wj6178o'
              	or $_GET['openid']=='oph7UwB2VfUzNThvK234uXmV3Pmk' or $_GET['openid']=='oph7UwD369svHO9Y7uZU5ivfcJh0'
                or $_GET['openid']=='oph7UwCn3GnC6W9vvZtAM1w3gugM' or $_GET['openid']=='oph7UwFoEjKD4jzUtMDhUbBShihA'
                or $_GET['openid']=='oph7UwBhlo1ZSneYCfvrv9OCUqCs'
                //
                ) {
                session('level',10);
            //前三位不是
...
//            }
    ...
    public function index(){
		...
		*/
                ...
                $todo = M('announcement')->where(array('id'=>I('get.aid')))->find();
                //
...
//                }
                ...
                if ($add) {
                    ...
                    $http = new \Think\Http;
                    /*$http->httpPost('https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.$indexA->getToken(TRUE), '{
                    "touser":"'.$this->userDetail($todo['signup'])['openid'].'",
                    "msgtype":"text",
                    "text": {
                    ...
                    $http->httpPost('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$indexA->getToken(TRUE), '{
                            "touser":"'.$this->userDetail($todo['signup'])['openid'].'",
                            "template_id":"7PjGlUtVpe0qGstISh91p3N5aM4wUzc87SZ2ThB1xpo",
                            "url":"www.kmic168.com/?m=web&a=artistProfiles&uid='.session('id').'",
                            "msgtype":"text",
                            "data":{
                                "first": {
                                    "value":"您好，您发布的通告有主持人报名，报价为：'.I('get.price').'元。以下是主持人信息",
                                    "color":"#173177"
                                },
                                "keyword1":{
                                    "value":"'.$this->userDetail(session('id'))['name'].'",
                                    "color":"#173177"
                                },
                                "keyword2": {
                                    "value":"'.$this->userDetail(session('id'))['mob'].'",
                                    "color":"#173177"
                                },
                                "keyword3": {
                                    "value":"'.I('get.title').'",
        ...
        if ($expire != NULL && time() > ($expire - 259200) && time() < $expire && S('member'.I('get.openid')) == null) {
            $http = new \Think\Http;  
            $http->httpPost('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$indexA->getToken(TRUE), '{
                    "touser":"'.I('get.openid').'",
                    "template_id":"-_rBXtJ8cYZyCdpp8AGdEMVCcklWAChcQMFAeSj1DCc",
                    "url":"www.kmic168.com/?m=Center&c=profile&a=member&openid='.I('get.openid').'",
                    "msgtype":"text",
                    "data":{
                        "first": {
                            "value":"您的会员即将过期",
            ...
            S('member'.I('get.openid'),"1",259200);
        }
...
//        }
    ...
    function contactSentToMsg($id) {
        ...
        'touid'=>session('id'),
        'msg'=>'通告《<a href="?m=web&a=announcementPage&annid='.$id.'&mine=mine">'.$announcement['title'].'</a>》的联系方式是:<input name="contact" type="text" style="width:100%; border:0" value="'.$contact.'"/>',
        'timeline'=>  time()
            ));
...
//        }
    ...
    public function deleteUserData() {
        ...
        if (session('level')==10) {
            if (M('user_detail')->where(array('uid'=>I('get.id')))->save(array('isshow'=>1))) {
                echo json_encode(array('code'=>0));
            }
        }
    }
...
//            echo 'systerm is Android';
    ...
    function wxSign(){
        ...
        \Think\Log::write('jsapi_ticket', S('jsapi_ticket')->ticket);
...
//        exit(); 
    ...
    public function goods(){
        ...
        if (isset($_GET['action']) && I('get.action')=='new') {
            $count = M('exchange')->where(array('uid'=>session('id')))->count();
            $member = M('users')->field('level')->where(array('id'=>session('id')))->find();

            if ($count == 3 && $member['level'] < 3) {
                echo json_encode(array('code'=>1,'msg'=>'非VIP会员最多只能发3个闲置物品。开麦不易，最好的支持，是您成为VIP。'));
            } else {
                echo json_encode(array('code'=>0));
            }
...
//CZC for goods details page start
    public function goodsPage() {
        ...
        $list = M('exchange')->where('id='.$_GET['gid'])->find();
        $picId = substr($list["pic_id"],1);

        $where['id']  = array('in',$picId);
        $list['exchange_pic'] = M('exchange_pic')->where($where)->find(); 

        $this->assign('gid', $_GET['gid']);
        $this->assign('name', $list['title']);
        $this->assign('price', $list['price']);
        $this->assign('img', $list['exchange_pic']['value']);
        $this->assign('signature',  $this->wxSign());
        $this->display();
    ...
    public function goodsDetails() {
                //get the announcement details using the id

        $list = M('exchange')->where('id='.$_GET['gid'])->find();

        $list['userDetail'] = M('users')->where('id='.$list['uid'])->find();
        $list['user_id'] = substr($list['userDetail']['addtime'], -3).$list['userDetail']['id'];

        $picId = substr($list["pic_id"],1);

        $where['id']  = array('in',$picId);
        $list['exchange_pic'] = M('exchange_pic')->where($where)->select();      

        $list['fromuid'] = session('id');
    ...
    public function goodsNew(){
        ...
        if (isset($_GET['action']) && I('get.action')=='addgoods') {
            ...
            $arr = array(
                ...
                'measurements'=>I('post.measurements'),
        ...
        if (isset($_GET['upload'])) {
            ...
            $pic_id = M('exchange_pic')->add(array('uid'=>$uid,'value'=>'http://www.kmic168.com/images/'.$file));
            if ($pic_id) {
        ...
        checkWX();    //Alpha debug remove the verify of wechat
        if(!isset($_GET['openid'])){
            $indexA = A('Center/Index');
            $indexA->weixinGetOpenid();    
            exit();
...
//CZC for goods details page end


//Alpha for annuncement details page start
    public function announcementPage() {
        ...
        $list = M('announcement')->where('id='.$_GET['annid'])->find();
        $this->assign('name', $list['title']);
        $this->assign('city', $list['city']);
        $this->assign('aid', $_GET['annid']);
        $this->assign('signature',  $this->wxSign());
        $this->display();
    ...
    public function announcementDetails() {
        ...
        $list['publish_user'] = M('users')->where('id='.$list['signup'])->find();
        
        $list['wx_info'] = object_array(json_decode(stripslashes($list['wx_info'])));
        $list['wx_info']['headimgurl']=$list['wx_info']['headimgurl']==''?'':$list['wx_info']['headimgurl'];
        $list['wx_info']['nickname']=$list['wx_info']['nickname']==''?'-':$list['wx_info']['nickname'];
...
//Alpha for annuncement details page end


    //function to jump to the artist detail page
    public function artistPage(){
        ...
        //     exit();
        // }
        $list = M('users')->where('id='.$_GET['uid'])->find();
        ...
        //to get the artist detail data
    public function artistDetails() {
        //get the user data;
        $list = M('users')->where('id='.$_GET['uid'])->find();
        $list['userDetail'] = M('user_detail')->where('uid='.$_GET['uid'])->find();

        $list['additional'] = M('user_additional')->where(array('uid='.$_GET['uid'],'type'=>'1'))->select();
        $list['artistVoice'] = M('user_additional')->where(array('uid='.$_GET['uid'],'type'=>'3'))->find();
        $list['userActivity'] = M('user_activity')->where('uid='.$_GET['uid'])->select();

        //handle the vaule of the data
        $list['time'] =  date("Y-m-d H:i:s",$list['timeline']) ;
        $list['language'] = json_decode($list['language']);
        $list['industry'] = json_decode($list['industry']);
        $list['skill'] = json_decode($list['skill']);
        $list['fromuid'] = session('id');
        if($list['headimgurl_user'] !=NULL){
             $list['headimgurl'] = $list['headimgurl_user'];
         }else if($list['headimgurl'] !=NULL){
        ...
        if($list['name_user'] !=NULL){
         $list['name'] = $list['name_user'];
        }                      

        $i = 0;
        $activityData = $list['userActivity'];
        foreach ($activityData as $activity) {
            ...
            $where['id']  = array('in',$picId);
            $activityData[$i]['img'] = M('user_additional')->where($where)->select();      
            $i++;
        }
        $list['userActivity'] = $activityData;

        unset($list['headimgurl_user']);
        unset($list['name_user']);

        header('Content-type: application/json');
        echo json_encode($list);
    }
//CZC for annuncement details page start
    

    

//CZC for annuncement details page end
...
//                        }
                        ...
                        M('messages')->add(array('fromuid'=>112,
                    'touid'=>$value['id'],
                    'msg'=>'VIP系统已把您推荐给通告[<a href="?m=web&a=announcementPage&annid='.$announcement.'&mine=mine">'.I('post.title').'</a>]',
                    'timeline'=>  time()
                        ));
...
//            }
    ...
    public function goodsList(){
        ...
        foreach ($whereExplode as $value) {
            ...
            if ($whereExplodeSub[0]=='size' && $whereExplodeSub[1] != '') {
                $where['size'] = $whereExplodeSub[1];
            ...
            if ($whereExplodeSub[0]=='price') {
                if ($whereExplodeSub[1] == '0') {
                    ...
                    $where['price'] = array(between,array(0,100));
                    $key .= 'price100';
                }else if ($whereExplodeSub[1] == '1-3') {
                    $where['price'] = array(between,array(100,300));
                    $key .= 'price1-300';
                }else if ($whereExplodeSub[1] == '3-10') {
                    $where['price'] = array(between,array(300,10000));
                    $key .= 'price3-10000';
                }
            }
            if ($whereExplodeSub[0]=='new') {
                if ($whereExplodeSub[1] == '10') {
                    $where['new'] = $whereExplodeSub[1];
                    $key .= 'new10';
                }else if ($whereExplodeSub[1] == '9-10') {
                    $where['new'] = array(between,array(9,10));
                    $key .= 'new9-10';
                }else if ($whereExplodeSub[1] == '8-9') {
                    $where['new'] = array(between,array(8,9));
                    $key .= 'new8-9';
                }else if ($whereExplodeSub[1] == '1') {
                    $where['new'] = $whereExplodeSub[1];
                    $key .= 'new1';
        ...
        S($key,NULL);
        if (S($key)==NULL) {
            $list = M('exchange')->where($where)->field('kwx_exchange.*,kwx_users.headimgurl,kwx_users.headimgurl_user,kwx_users.name_user,kwx_users.name')->join('kwx_users ON kwx_users.id = kwx_exchange.uid')->limit(I('get.page')*10,10)->order('modify_time desc')->select();


            $i = 0;
            $exchangeData = $list;
            foreach ($exchangeData as $exchange) {
                $picId = substr($exchange["pic_id"],1);
                if ($picId == NULL || $picId == ' ') {
                    $i++;
                    continue;
                }
                $where_pic['id']  = array('in',$picId);

                $exchangeData[$i]['exchange_pic'] = M('exchange_pic')->where($where_pic)->select();      
                $i++;
            }
            $list = $exchangeData;
...
//                    $key .= $whereExplodeSub[0].$whereExplodeSub[1];
            ...
            if ($whereExplodeSub[0]=='price') {
                if ($whereExplodeSub[1] == '0') {
                    ...
                    $where['price'] = array(between,array(3000,10000));
                    $key .= 'price3-10000';
                }
...
//                $where['price'] = array(between,array(1000,2000));
        ...
        //$where['timeline']=array('NEQ','NULL'); 这个不用置顶时候用
        $where['isshow'] = 0; //是不是麦主
...
//                } 
            $list = M('user_detail')->where($where)->field('kwx_user_detail.*,kwx_users.openid,kwx_users.headimgurl,kwx_users.headimgurl_user,kwx_users.name_user,kwx_users.level,kwx_users.openid,kwx_users.name,kwx_users.sex')->join('kwx_users ON kwx_users.id = kwx_user_detail.uid')->
                limit(I('get.page')*10,10)->order('FIELD (kwx_users.id,29232,27139,29232,24499,10450,33382,33277,33960,24967,33918) desc,kwx_user_detail.timeline desc')->select();  
...
//                $list[$key]['nickname']=
    ...
    public function messages() {
        if (isset($_POST)) {
            if (I('post.action')=='send') {
                ...
                'msg'=>I('post.msg'),
                'timeline'=>  time());
            } else if (I('post.action')=='focus') {
                $data = array('fromuid'=>I('post.fromuid'),
                'touid'=>I('post.touid'),
                'msg'=> '我对您发布的<a style="display:block;" href="?m=web&a=goodsPage&gid='.I('post.id').'">'.I('post.title').'</a>有兴趣<p>'.I('post.msg').'</p>',
                'timeline'=> time());
            }
            ...
            if ($add) {
                ...
                //         "msgtype": "text", 
                //         "text":{
                //             "content":"'.$this->userDetail(I('post.fromuid'))['name'].' 给您留言啦：'.I('post.msg').'。 <a href=\'www.kmic168.com/?m=Center&c=profile&a=message\'>点击这里</a>查看更多消息"
                //         }
                //     }'
                // );
                ...
                $http->httpPost('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$indexA->getToken(TRUE), '{
                        "touser":"'.$this->userDetail(I('post.touid'))['openid'].'",
                        "template_id":"BSfD2x5Ky4jVJaSFDJcKpImbrpCHNbHbjeyRqlT9Tfw",
                        "url":"www.kmic168.com/?m=Center&c=profile&a=message",
                        "msgtype":"text",
                        "data":{
                            "first": {
                                "value":"您好!有主持人给您留言了。",
                            ...
                            "user":{
                                "value":"'.$this->userDetail(I('post.fromuid'))['name'].'",
                                "color":"#173177"
                            },
                            "ask": {
                                "value":"'.I('post.msg').'",
    ...
    public function mine(){
        ...
        if (isset($_GET['list'])) {
            ...
            S($key,NULL);
            if (S($key)==NULL) {
                $list = M('announcement')->where(array('signup'=>session('id'),'is_show'=>1))->limit(I('get.page')*10,10)->order('id desc')->select();
...
//                exit();
    ...
    public function mineExchange(){
        ...
        // czc
        if (isset($_POST) && I('post.action')=='end') {
            M('exchange')->where(array('id'=>I('post.id')))->save(array('status'=>1));
            exit(json_encode(array("error"=>0,'msg'=>'')));
        }
        if (isset($_POST) && I('post.action')=='find') {
            M('exchange')->where(array('id'=>I('post.id')))->save(array('status'=>2));
            exit(json_encode(array("error"=>0,'msg'=>'')));
        }
        if (isset($_POST) && I('post.action')=='delete') {
            M('exchange')->where(array('id'=>I('post.id')))->save(array('is_show'=>0));
            exit(json_encode(array("error"=>0,'msg'=>'')));
        }
        
        if (isset($_GET['list'])) {
            $key = session('id').I('get.page').'uid';
            $where = array();
            $whereExplode = explode(',', I('get.where'));

            foreach ($whereExplode as $value) {
                $whereExplodeSub = explode('=', $value);
                if ($whereExplodeSub[0]=='is_show') {
                    $where['is_show'] = $whereExplodeSub[1];
                    $key .= $whereExplodeSub[0].$whereExplodeSub[1];
                }
            }
            ...
            if (S($key)==NULL) {
                ...
                $list = M('exchange')->where(array('uid'=>session('id'),$where))->limit(I('get.page')*10,10)->order('modify_time desc')->select();
...
//            print_r($_REQUEST);
	\Think\Log::write('post_json', json_encode($_REQUEST));
...
//            var_dump($centerUserData);
        ...
        //
        $wxData = M('users')->where(array('id'=>session('id')))->find();
        if($wxData['headimgurl_user']!=""){
            $wxData["headimgurl"] = $wxData['headimgurl_user'];            
        }
        
        if($wxData['name_user']!=""){
            $wxData["nickname"] = $wxData['name_user'];            
        }else{
            $wxData["nickname"] = $wxData['name'];            
        }
