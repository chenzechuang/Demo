...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function Queryorder($transaction_id)
	{
			...
			&& $result["result_code"] == "SUCCESS")
		{
                    ...
                    $user = M('users')->where(array('id'=>$result['attach']))->find();
                    if ($user['expire']!=null) {
                        if ($result['total_fee'] == '1000') {
...
//                    $userCenter = M('users','center_')->where(array('openid'=>$result['openid']))->find();
                    if ($user['not_first']==0) {
                        M('users')->where(array('id'=>$result['attach']))->save(array('level'=>3,'expire'=>$expire,'not_first'=>1));
//                        session('level',3);
//                        //第一次给代理钱
//                        //给第一个
...
//                        }
                        ...
                        M('users')->where(array('id'=>$result['attach']))->save(array('level'=>3,'expire'=>$expire));
//                        session('level',3);
                        //不是第一次
                    }
