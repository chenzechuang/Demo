...
class PushController extends Controller {
    ...
    private $app_key = '2fa9f1563ee185e5ff396fd6';        //待发送的应用程序(appKey)，只能填一个。
    private $master_secret = '28323fe79df059b5de49c92b';    //主密码
    ...
    public function index() {
        $apiCrl = A('Api/Push');
        echo $apiCrl->pushMsg('179', '小龙是天才','event','parameters');//userid , 内容  , 事件 ,参数
    }

    public function pushMsg($userid, $content,$m_type,$m_txt) {
        $model = D('Api/Push');
        $useridArr = $model->getRegistrationId($userid);
        if ($useridArr == null) {
            $message = '没有这个id';
            $json_data["state"] = 1;
            goto end;
...
//        $m_type = 'here is type'; //推送附加字段的类型
//        $m_txt = 'here is value'; //推送附加字段的类型对应的内容(可不填) 可能是url 带http:、、,可能是一段文字。
        $m_time = '86400'; //离线保留时间
        $receive = array('registration_id' => $model->getRegistrationId($userid));
//        var_dump($receive);
//        exit();
        $message = ""; //存储推送状态
        ...
        if ($result) {
            ...
            if (isset($res_arr['error'])) {                       //如果返回了error则证明失败
                ...
                M('syslogs', 'center_')->add(array('msg' => $res_arr['error']['message']));
...
//推送的Curl方法
    ...
    public function pushRegistration() {
        ...
        $controller->encryptionAPI(array('registration_id'), I('post.token'), I('post.timestamp'));

        if (strlen($_POST['registration_id']) > 2) {
            $model = D('Api/Push');
            echo $model->saveID($_POST['registration_id']);
        } else {
            $json_data["state"] = 1;
            $json_data['msg'] = 'registration_id 不正确';
