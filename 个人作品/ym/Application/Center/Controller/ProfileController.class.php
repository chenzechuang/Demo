...
//        $this->assign('agent', $agent);
        $user = M('users')->where(array('openid'=>I('get.openid')))->order('id desc')->find();
        ...
        if (isset($_GET['action'])&&I('get.action')=='msg') {  //我的资料
                ...
                if($openid){
                    ...
                    if(count($detail)>0){
                       ...
                       $up_detail =  M('user_detail')->where(array('uid'=>$user['id']))->add($post_data); 
                    }
        ...
        if ($_GET['action'] == 'verify') {
            if (strlen(I('post.mob'))==11) {
                ...
                if ($xmlToArr['returnsms']['returnstatus']=='Success') {
                    M('config')->where(array('id'=>10))->save(array('value'=>$xmlToArr['returnsms']['remainpoint']));
...
//        $this->display();
    ...
    public function message() {
        ...
        $mine = M('users')->where(array('openid'=>session('openid')))->find();
//        echo $mine['id'];
//        exit(); 
        $list = M('messages')->where('(touid = '.$mine['id'] .' or fromuid =  '.$mine['id'] .') and  mainid is null')->field('kwx_users.name,kwx_messages.*')->join('kwx_users ON kwx_users.id =  kwx_messages.fromuid')->order('kwx_messages.timeline desc')->limit('80')->select();
//        var_dump($list);
//        exit(); 
        $webController = A('Web/Index');
        foreach ($list as $key => $value) {
            $subList = M('messages')->where('mainid = '.$value['id'])->field('kwx_users.name,kwx_messages.*')->join('kwx_users ON kwx_users.id =  kwx_messages.fromuid')->order('id desc')->limit('80')->select();
