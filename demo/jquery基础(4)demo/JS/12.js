...
$(document).ready(function() {
	var $table1 = $('#t-1');
	var $headers1 = $table1.find('thead th').slice(1);
	$headers1
		.each(function() {
	...
	$headers1.click(function(event) {
		...
		$headers1.removeClass('sorted-asc sorted-desc');
...
//HTML5
	var $table2 = $('#t-2');
	var $headers2 = $table2.find('thead th').slice(1);
	$headers2
		.wrapInner('<a href="#"></a>')
		.addClass('sort');
	var rows = $table2.find('tbody > tr').get();
	$headers2.click(function(event) {
		event.preventDefault();
		var $header = $(this),
		...
		$headers2.removeClass('sorted-asc sorted-desc');
		$header.addClass(sortDirection == 1 ? 'sorted-asc' : 'sorted-desc');
		$.each(rows, function(index, row) {
